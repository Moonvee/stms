 ...
 * @return string
 */
function show_status1($status, $id, $field = 'id', $controller = '')
{
    $controller === '' && $controller = Request::instance()->controller();
    switch ($status) {
        // 恢复
        case 0 :
            $ret = '<a href="javascript:;" onclick="ajax_req(\'' . Url::build($controller . '/resume1', [$field => $id]) . '\',{},change_status,[this,\'resume1\'])" class="label label-warning radius" title="点击标记为已测试">未测试</a>';
            break;
        // 禁用
        case 1 :
            $ret = '<a href="javascript:;" onclick="ajax_req(\'' . Url::build($controller . '/forbid1', [$field => $id]) . '\',{},change_status,[this,\'forbid1\'])" class="label label-success radius" title="点击标记为未测试">已测试</a>';
            break;
        // // 还原
        // case -1 :
        //     $ret = '<a href="javascript:;" onclick="ajax_req(\'' . Url::build($controller . '/recycle', [$field => $id]) . '\')" class="label label-secondary radius" title="点击还原">还原</a>';
...
function get_status1($status, $imageShow = true)
{
    switch ($status) {
        case 0 :
            $showText = '未测试';
            $showImg = '<i class="Hui-iconfont c-warning status" title="未测试">&#xe631;</i>';
            break;
        // case -1 :
        //     $showText = '删除';
        //     $showImg = '<i class="Hui-iconfont c-danger status" title="删除">&#xe6e2;</i>';
        ...
        default :
            $showText = '已测试';
            $showImg = '<i class="Hui-iconfont c-success status" title="已测试">&#xe615;</i>';
    }
    return ($imageShow === true) ? $showImg : $showText;
}
